...
@interface LTxNetworkDownloadService()<NSURLSessionDelegate>
...
@end
@implementation LTxNetworkDownloadService

/**
 * 单例模式
 **/
static LTxNetworkDownloadService *_instance;
+ (instancetype)sharedInstance{
    static dispatch_once_t onceTokenLTxNetworkDownloadService;
    dispatch_once(&onceTokenLTxNetworkDownloadService, ^{
        _instance = [[LTxNetworkDownloadService alloc] init];
        [LTxNetworkDatabase tablesInit];
...
-(void)addDownloadTaskWithURL:(NSString*)url pathInSandbox:(NSString*)path saveName:(NSString*)saveName{
    //开始下载
    NSURL* fileURL = [NSURL URLWithString:[url stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]]];
    if (!fileURL) {
        return;
    ...
    NSURLSessionDownloadTask* downloadTask = [self.session downloadTaskWithURL:fileURL];
    downloadTask.taskDescription = url;
    [LTxNetworkDatabase addDownloadTaskWithURL:url pathInSandbox:path name:saveName];//缓存起来
    [self startDownloadServiceWithTask:downloadTask];
}
...
//    NSString* cacheFilePath = [[NSHomeDirectory() stringByAppendingPathComponent:@"Library/Caches"] stringByAppendingPathComponent:lastPathComponent];
//
...
//    }
    ...
    NSDictionary* taskItem = [LTxNetworkDatabase downloadTaskWithURL:downloadTask.taskDescription];
    if (taskItem) {
        ...
        [fileManager moveItemAtPath:location.path toPath:fileAbsolutePath error:&error];
        if (!error) {
            NSLog(@"文件移动成功:%@",fileAbsolutePath);
        }else{
            NSLog(@"文件移动失败：%@",error.description);
        }
        [LTxNetworkDatabase finishDownloadTaskWithURL:downloadTask.taskDescription];
    }
